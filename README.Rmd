...
```{r, bhs_hist}
dim(bhs_subset)
range(table(bhs_subset$mci))
hist(table(bhs_subset$mci), main='Number of BHS entries per individual')
```
...
```{r, cyf_hist}
dim(cyf_subset)
range(table(cyf_subset$mci))
hist(table(cyf_subset$mci), main='Number of homeless shelter entries per individual')
```
...
```{r, shelters_hist}
dim(shelters_subset)
range(table(shelters_subset$mci))
hist(table(shelters_subset$mci), main='Number of homeless shelter entries per individual')
```
